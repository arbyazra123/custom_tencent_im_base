...
pathList = ["lib/i18n/strings_zh-Hans.i18n.json"];
...
Promise.all(
  ...
  try{
      // 增补韩语JSON
      const hantDataFile = fs.readFileSync('lib/i18n/strings_ko.i18n.json');
      const hantData = JSON.parse(hantDataFile);
    ...
    try{
        // 增补日语JSON
        const hantDataFile = fs.readFileSync('lib/i18n/strings_ja.i18n.json');
        const hantData = JSON.parse(hantDataFile);
